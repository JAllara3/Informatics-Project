...
<?php
	...
	if(isset($_SESSION['email'])) {
		...
		if (nTuples($result_3) > 0) {
			...
			$cartid = $_SESSION['cartid'];
			$query_1 = "SELECT DISTINCT products.name as name, productorder.amount as amount, products.prices as prices, productorder.id as Pid, productorder.productsid as Cid FROM products, productorder, carts WHERE productorder.status = 'cart' AND productorder.cartid = carts.id AND carts.userid = ". $_SESSION['userid'] ." AND productorder.productsid = products.id;";
			...
			if (!isset($_SESSION['cartid'])) {
				...
				$query_4 = "SELECT DISTINCT products.name as name, productorder.amount as amount, products.prices as prices, productorder.id as Pid, productorder.productsid as Cid FROM products, productorder, carts WHERE productorder.status = 'cart' AND productorder.cartid =". $_SESSION['cartid'] ." AND productorder.productsid = products.id;";
